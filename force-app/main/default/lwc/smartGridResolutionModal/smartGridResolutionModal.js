...
export default class SmartGridResolutionModal extends LightningElement {
  ...
  get currentRecord() {
    if (
      this.failedRecords.length > 0 &&
      this.currentIndex < this.failedRecords.length
    ) {
  ...
  get fieldsToRender() {
    ...
    return this.columns
      .filter(
        (c) =>
          c.fieldName && c.fieldName !== "Id" && !c.fieldName.endsWith("_Url")
      )
      .map((c) => {
        return {
          fieldName: c.fieldName,
          value:
            this.currentRecord[c.fieldName] !== undefined
              ? this.currentRecord[c.fieldName]
              : null
        };
      });
  }
